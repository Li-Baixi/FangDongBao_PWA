...
-- =============================================================
-- 行级安全（RLS）：成员只能看/改自己的数据；管理员可以看/改全部。
-- 业务表的 ownerId 存的是 landlords.id（档案 ID），经 is_my_data 映射到登录账号。
-- =============================================================
...
-- 业务表的 ownerId 存的是 landlords.id（前端生成的档案 ID），
-- 权限判断要把"档案"映射回"登录账号"再比对。
create or replace function public.is_my_data(p_owner uuid)
returns boolean
language sql
stable
security definer
...
begin
  ...
  loop
    ...
    execute format('create policy "%s_delete" on public.%I for delete to authenticated using (public.is_my_data("ownerId") or public.is_admin());', t, t);
  end loop;
end;
$$;
